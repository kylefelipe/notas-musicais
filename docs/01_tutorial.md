...
#### Variações nas escala

Pra alterar a escala, você pode alterar os parâmetros: `tônica` e `tonalidade`. Eles são chamados na seguinte ordem:
...
Avariação da tônica pode ser feita em qualquer nota. Por exemplo, se quisermos alteração da escala de Fá sustenido:
